...
async def start_migration(
    ...
    background_tasks.add_task(
        ...
        task_id, 
        file,
        university_id or os.getenv("DEFAULT_UNIVERSITY_ID", "default_univ"),
        author_id or os.getenv("DEFAULT_AUTHOR_ID", "default_author")
    )
...
class S3MigrateRequest(BaseModel):
    ...
    university_id: Optional[str] = Field(None, description="University ID for scoping (optional)")
    author_id: Optional[str] = Field(None, description="Author ID for scoping (optional)")
...
async def start_migration_from_s3(
    ...
    background_tasks.add_task(
        ...
        task_id,
        body.s3_key,
        body.university_id or os.getenv("DEFAULT_UNIVERSITY_ID", "default_univ"),
        body.author_id or os.getenv("DEFAULT_AUTHOR_ID", "default_author"),
        body.bucket,
    )
...
class CanvasMigrateRequest(BaseModel):
    course_id: str = Field(..., description="The Canvas Course ID to migrate via API")
    university_id: Optional[str] = Field(None, description="University ID for scoping")
    author_id: Optional[str] = Field(None, description="Author ID for scoping")
    force: bool = Field(False, description="Bypass deduplication and force re-import")


@router.post("/migrate-canvas")
async def start_canvas_migration(
    body: CanvasMigrateRequest,
    background_tasks: BackgroundTasks,
    api_key: str = Depends(get_api_key)
) -> Dict[str, Any]:
    """
    Trigger a migration by extracting data directly from the Canvas API.
    """
    task_id = str(uuid.uuid4())

    background_tasks.add_task(
        migration_service.process_canvas_migration,
        task_id,
        body.course_id,
        body.university_id,
        body.author_id,
